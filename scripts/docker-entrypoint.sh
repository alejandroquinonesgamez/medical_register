...
# Asi se evita tener que usar chmod 777 en el host al desplegar.

set -e

# Directorios que la aplicacion debe poder escribir (bind mount .:/app)
mkdir -p /app/data /app/data/temp /app/data/wstg_sync_queue /app/docs/informes /app/instance

# Ajustar dueno para que el proceso no root pueda escribir (UID 1000 = appuser)
...
# Asi las variables se leen aunque Docker Compose no las inyecte (p. ej. path con espacios).
# Ejecutar como appuser (init_storage + gunicorn); su esta en toda imagen Debian
